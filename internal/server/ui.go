...
const configPageTemplateSV = `<!DOCTYPE html>
<html lang="sv">
<head>
  <meta charset="UTF-8">
...
const configPageTemplateEN = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>ReCal - Configure</title>
  <style>
    body {
      font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
      max-width: 800px;
...
<body>
  <div class="container">
    <h1>ReCal</h1>
    <p class="subtitle">Configure your calendar filters</p>
    ...
    <div class="filter-section">
      ...
      <p class="help-text">
        Keeps the selected grade and below, filters out higher grades
      </p>
    </div>
    ...
    <div class="filter-section">
      <h3>Lodges</h3>
      <div class="controls">
        <button id="select-all-lodges">Select all</button>
        <button id="deselect-all-lodges">Deselect all</button>
      </div>
      <div class="checkbox-list" id="loge-checkboxes">
        <div class="loading">Loading lodges...</div>
      </div>
      <p class="help-text">
        Deselect lodges to filter them out
      </p>
    </div>
    ...
    <!-- Calendar App Integration -->
    <div class="filter-section">
      <h3>Open in calendar app</h3>
      <div class="action-buttons" id="calendar-apps">
        <!-- Populated by JavaScript based on platform detection -->
      </div>
      <p class="help-text">
        Choose a calendar app to subscribe to the filtered calendar
      </p>
    </div>
  </div>
  ...
  <script>
    ...
    async function loadLodges() {
      try {
        ...
        document.getElementById('loge-checkboxes').innerHTML =
          '<div class="loading">Could not load lodges: ' + err.message + '</div>';
      }
    }
    ...
    function setupCalendarApps() {
      ...
      const webcalBtn = document.createElement('button');
      webcalBtn.className = 'btn-secondary';
      webcalBtn.innerHTML = '📱 Other app (webcal://)';
      webcalBtn.onclick = () => {
        const webcalURL = getSubscriptionURL();
        navigator.clipboard.writeText(webcalURL).then(() => {
          alert('webcal:// URL copied to clipboard!\\n\\nPaste into your calendar app.');
        }).catch(() => {
          prompt('Copy this webcal:// URL into your calendar app:', webcalURL);
        });
      };
      container.appendChild(webcalBtn);
    }
